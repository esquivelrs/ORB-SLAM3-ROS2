...
std::unordered_map<long unsigned int, ORB_SLAM3::KeyFrame *> ORBSLAM3Interface::makeKFIdPair(
  std::vector<ORB_SLAM3::Map *> mapsList)
{
    ...
    if (c == 0) {
      ...
      if (mapReferencePoses_.count(allKFs_[parentMapID]->GetMap()) == 0) {
        throw std::runtime_error(
                "The parent map pose for this map ID does not exist. This should not happen.");
      }
      auto parentMapORBPose = allKFs_[parentMapID]->GetPose();
      mapReferencePoses_[mapsList[c]] =
        typeConversions_->transformPoseWithReference<Eigen::Affine3d>(
        mapReferencePoses_[allKFs_[
          parentMapID]->GetMap()], parentMapORBPose);
    }
  }
  // std::cout << "+++++++++++++++++++++++++++++++++" << std::endl;
  ...
  for (auto & KF : atlasAllKFs_) {
    for (auto & mapPoint : KF->GetMapPoints()) {
      if (!mapPoint->isBad()) {
        ...
        auto mapPointWorld = typeConversions_->transformPointWithReference<Eigen::Vector3f>(
          mapReferencePoses_[allKFs_[KF->mnId]->GetMap()], worldPos);
        mapReferencesMutex_.unlock();
...
void ORBSLAM3Interface::mapPointsVisibleFromPose(
  geometry_msgs::msg::Pose cameraPose,
  std::vector<ORB_SLAM3::MapPoint *> & points,
  int maxLandmarks, float maxDistance,
  float maxAngle)
{
  ...
  float maxAngle)
{
  ...
  if (orbAtlas_->GetAllCameras().size() > 1) {
    throw std::runtime_error(
            "The number of cameras is greater than 1. This will be catered later. You cannot check if a map point is visible within a keyframe in this version. Check if there have been version updates.");
  }
  ...
  for (auto pKFMp : mapKFs_) {
    ...
    if (distBwKfs > maxDistance) {
      ...
      if (rel_T_CamToKF.translation()[2] < 0.0) {
        ...
        if ((relEulerAngles(2) >= 0 && relEulerAngles(2) < M_PI / 2.0) ||
          (relEulerAngles(2) > 3.0 * M_PI / 2.0 && relEulerAngles(2) < 2 * M_PI))
        {
          continue;
        }
      }
...
void ORBSLAM3Interface::mapDataToMsg(
  slam_msgs::msg::MapData & mapDataMsg, bool currentMapKFOnly,
  bool includeMapPoints, std::vector<int> kFIDforMapPoints)
{
      ...
      if (allKFs_.find(kFId) != allKFs_.end()) {
        for (auto mapPoint : allKFs_[kFId]->GetMapPoints()) {
          if (!mapPoint->isBad()) {
            ...
            mapReferencesMutex_.lock();
            auto mapPointWorld =
              typeConversions_->transformPointWithReference<geometry_msgs::msg::Point>(
              mapReferencePoses_[allKFs_[kFId]->GetMap()], worldPos);
            mapReferencesMutex_.unlock();
...
void ORBSLAM3Interface::getDirectMapToRobotTF(
  std_msgs::msg::Header headerToUse,
...
void ORBSLAM3Interface::getMapToOdomTF(
  const nav_msgs::msg::Odometry::SharedPtr msgOdom,
  geometry_msgs::msg::TransformStamped & tf)
{
  // tf.header.stamp;
  tf.header.frame_id = globalFrame_;
...
void ORBSLAM3Interface::getOptimizedPoseGraph(
  slam_msgs::msg::MapGraph & graph,
  bool currentMapKFOnly)
{
  if (!currentMapKFOnly) {
    for (const auto & cKf : allKFs_) {
      ...
      mapReferencesMutex_.lock();
      kfPoseStamped.pose = typeConversions_->transformPoseWithReference<geometry_msgs::msg::Pose>(
        mapReferencePoses_[kf->GetMap()], kfPose);
      mapReferencesMutex_.unlock();
    ...
    for (auto pKFcurr : vKeyFrames) {
      ...
      poseStamped.pose = typeConversions_->transformPoseWithReference<geometry_msgs::msg::Pose>(
        currReferencePose_, kFPose);
...
bool ORBSLAM3Interface::trackRGBDi(
  const sensor_msgs::msg::Image::SharedPtr msgRGB,
  ...
  Sophus::SE3f & Tcw)
{
  ...
  if (!imuBuf_.empty()) {
    ...
    vImuMeas.clear();
    while (!imuBuf_.empty() &&
      typeConversions_->stampToSec(imuBuf_.front()->header.stamp) <=
      std::min(
        typeConversions_->stampToSec(msgRGB->header.stamp),
        typeConversions_->stampToSec(msgD->header.stamp)))
    {
      double t = typeConversions_->stampToSec(imuBuf_.front()->header.stamp);
      cv::Point3f acc(imuBuf_.front()->linear_acceleration.x,
        imuBuf_.front()->linear_acceleration.y, imuBuf_.front()->linear_acceleration.z);
  ...
  if (imuBuf_.size() > 0) {
    ...
    Tcw =
      mSLAM_->TrackRGBD(
      cvRGB->image, cvD->image, typeConversions_->stampToSec(
        msgRGB->header.stamp), vImuMeas);
...
bool ORBSLAM3Interface::trackRGBD(
  const sensor_msgs::msg::Image::SharedPtr msgRGB,
  const sensor_msgs::msg::Image::SharedPtr msgD, Sophus::SE3f & Tcw)
{
  orbAtlas_ = mSLAM_->GetAtlas();
  cv_bridge::CvImageConstPtr cvRGB;
  cv_bridge::CvImageConstPtr cvD;
...
bool ORBSLAM3Interface::trackMONO(
  const sensor_msgs::msg::Image::SharedPtr msgRGB,
  Sophus::SE3f & Tcw)
{
  orbAtlas_ = mSLAM_->GetAtlas();
  ...
  if (currentTrackingState == 2) {
    ...
    switch (currentTrackingState) {
      case 0:
        ...
        break;
      case 1:
        std::cerr << "ORB-SLAM failed: Not initialized MONO." << endl;
        break;
      case 3:
        std::cerr << "ORB-SLAM failed: Tracking LOST." << endl;
        break;
